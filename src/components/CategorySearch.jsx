...
//     }
//     setError("");
//     navigate(`/category/${encodeURIComponent(category.trim().toLowerCase())}`);
//   };
...
//           value={category}
//           onChange={(e) => setCategory(e.target.value)}
//           placeholder="Enter category (e.g. electronics, laptops, jewelery)"
//           className="flex-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
//         />
...
const CategorySearch = () => {
  ...
  const handleSearch = () => {
    ...
    setError("");
    navigate(`/search/${encodeURIComponent(category.trim().toLowerCase())}`); // ✅ Safe encode
  };
